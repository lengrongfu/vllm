...
@triton.jit
def _gemma_rms_norm(x, w, eps, HIDDEN_SIZE: tl.constexpr):
    x = x.to(tl.float32)
    mean_sq = tl.sum(x * x, axis=0) / HIDDEN_SIZE
    rrms = tl.rsqrt(mean_sq + eps)
    return (x * rrms) * (1.0 + w.to(tl.float32))


@triton.jit
def _fused_eh_gemma_norm_kernel(
    pos_ptr,
    embeds_ptr,
    embeds_stride,
    prev_ptr,
    ...
    """MTP input fusion: zero embeds at position 0, Gemma RMSNorm(embeds) with
    enorm and Gemma RMSNorm(prev_hidden) with hnorm, written side-by-side into
    ``out`` ([N, 2H]) ready for the eh_proj GEMM. Replaces where + 2x
    gemma_rmsnorm + cat."""
    tok = tl.program_id(0)
    ...
    e_normed = _gemma_rms_norm(e, ew, eps, H)
    ...
    p_normed = _gemma_rms_norm(p, hw, eps, H)
    tl.store(out_ptr + tok * out_stride + H + off, p_normed, mask=mask)


def fused_eh_gemma_norm(
